...
if ($result_disciplinas->num_rows > 0) {
    while ($row_disciplina = $result_disciplinas->fetch_assoc()) {
        ...
        if ($filtro === 'mes') {
            $sql_pagos = "SELECT id_cliente, nombre, apellido, disciplina, fecha_pago, monto, tipo_pago FROM pagos WHERE MONTH(fecha_pago) = MONTH('$fecha_actual') AND YEAR(fecha_pago) = YEAR('$fecha_actual') AND disciplina = '$disciplina_actual'";
        } elseif ($filtro === 'dia') {
            $sql_pagos = "SELECT id_cliente, nombre, apellido, disciplina, fecha_pago, monto, tipo_pago FROM pagos WHERE DATE(fecha_pago) = '$fecha_actual' AND disciplina = '$disciplina_actual'";
        } else {
